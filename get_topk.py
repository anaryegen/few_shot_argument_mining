...
if __name__ == "__main__":
    ...
    if args.multitoken:
        label_map_output = {label:list(tokens.keys()) for label, tokens in label_map.items()}
    else:
        label_map_output = {label: list(tokens.keys())[0] if len(tokens.keys()) > 0 else "" for label, tokens in label_map.items()}
