...
def collect_label_token_from_data(data_path, label_list, args, dataset='conll', max_seq_length=128, batch_size=64, device='cuda', model_path='/home/jotion/NER/few-shot/bert/models', save_path=None, tokenizer=None, model=None):
    ...
    print('### loading data ###')
     # 统计高频词时不区分BI 
    train_examples = processor.get_examples(data_path, schema='BIO', label_type=args.label_type)
...
if __name__ == '__main__':
    ...
    collect_label_token_from_data(data_path, label_list, args, dataset=args.dataset,max_seq_length=128, batch_size=64, device='cpu', model_path=args.model_path, save_path=save_path)
    
    label_frac = json.load(open(save_path, 'r'))
    label_map = build_label_vocabulary(label_frac)
    label_map = {'{}'.format(key):value for key,value in label_map.items()}
    print(label_map)
