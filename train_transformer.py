...
def main():
    ...
    if args.label_map_path is not None:
        ...
        ori_label_token_map = {"PER":['Michael', 'John', 'David', 'Mark', 'Martin', 'Paul'], "ORG": ['Inc', 'Co', 'Corp', 'Party', 'Association', '&'],
                           "LOC":['Germany', 'Australia', 'England', 'France', 'Italy', 'Belgium'], "MISC":['German', 'Cup', 'French', 'Israeli', 'Australian', 'Olympic']}
    ...
    label_list += 'O'
    print("ORI_LABEL_TOKEN_MAP", ori_label_token_map)
    label_to_id = {"O":0}
    for l in label_list:
        if l != "O":
    ...
    if True: #args.eval_label_schema == "BIO":
        ...
        for id, label in enumerate(label_to_id.keys()):
    ...
    if "roberta" in args.model_name_or_path:
        ...
        tokenizer = add_label_token_deberta(model, tokenizer, ori_label_token_map)
        ...
        for i, label in enumerate(examples[label_column_name]):
            ...
            for input_idx, word_idx in zip(input_ids, word_ids):
                ...
                # Set target token for the first token of each word.
                    ...
                    if args.label_schema == "BIO" and label[word_idx] != "O":
                        label[word_idx] = label[word_idx]
    ...
    label_id_list = torch.tensor([label_token_to_id[id_to_label[i]] for i in range(len(id_to_label)) if i != 0], dtype=torch.long, device=device)
    ...
    def evaluate(best_metric, load=False, use_crf=False):
        ...
        write_results = open(os.path.join(args.output_dir, "eval_results.txt"), 'a')
        print("Decoding time: {}s".format(time.time() - start))
        # accelerator.print(f"epoch {epoch}:", eval_metric)
        for key in eval_metric.keys():
            if "f1" in key and "overall" not in key:
                label = key[:-3]
                write_results.write("{}: {}, \n {}: {},\n {}: {},\n {}: {} \n".format(label + "_precision", eval_metric[label + "_precision"],
        ...
        fname = f'predictions_{str(idx)}.txt'
...
def add_label_token_deberta(model, tokenizer, label_map):
    sorted_add_tokens = sorted(list(label_map.keys()), key=lambda x: len(x), reverse=True)
    tokenizer.add_tokens(sorted_add_tokens)
    num_tokens, _ = model.deberta.embeddings.word_embeddings.weight.shape
    model.resize_token_embeddings(len(sorted_add_tokens)+num_tokens)
    for token in sorted_add_tokens:
        if token.startswith('B-') or token.startswith('I-'):  # 特殊字符
            index = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(token))
            ...
            if isinstance(label_map[token], list):
                ...
                indexes = tokenizer.convert_tokens_to_ids([label_map[token]])
            embed = model.deberta.embeddings.word_embeddings.weight.data[indexes[0]]

            # Calculate mean vector if there are multiple label words.
            for i in indexes[1:]:
                embed += model.deberta.embeddings.word_embeddings.weight.data[i]
            embed /= len(indexes)
            model.deberta.embeddings.word_embeddings.weight.data[index] = embed
